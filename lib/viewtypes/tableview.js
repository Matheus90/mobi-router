(function(){
    ...
    mobiTableviewLinkEvents[END_EV+' .tableview-link'] = function(e) {
        $('.touched').removeClass('touched');
        var target = e.target;
        while (target.className != 'btn-container' && !target.className.match('tableview-link') ) target = target.parentNode;
        if(_.isFunction(this.action) && !mobileScrolling && $(target).hasClass('tableview-link'))
            this.action(e);
    };
    ...
    mobiTableviewPressableEvents[START_EV+' .tableview-pressable'] = function(e){
        var target = e.target;
        while (!target.className.match('btn-container') && !target.className.match('tableview-pressable') ) target = target.parentNode;
        if( $(target).hasClass('tableview-pressable') )
            $(e.currentTarget).addClass('touched');
    };
    mobiTableviewPressableEvents[END_EV+' .tableview-pressable'] = function(e){
        ...
        var target = e.target;
        while (!target.className.match('btn-container') && !target.className.match('tableview-pressable') ) target = target.parentNode;
        if(_.isFunction(this.titleAction) && !mobileScrolling && $(target).hasClass('tableview-pressable'))
            this.titleAction(e);
    };
