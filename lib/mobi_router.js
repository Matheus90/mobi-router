(function($) {
    MRouter = function(){
		...
		function _getSequenceFromUrl(url) {
			...
			console.log('SEQUENCE', route, _routeMap);
		...
		function _findCurrentRoute(routeMap, url){ 
			...
			console.log('BEST MATCH', bestMatchingRoute);
        ...
        function _calculateUrl(sequence){
			...
			console.log("CALC", String(url ? url : '/'), _sequence);
        ...
        this.configure = function(settings){
            ...
            if(isMobile) {
				...
				var self = this;
				$(function() {
					self.handleKeyboardForInputsInScroller(); //ADDED BY JAMES -- JUNE 15 2014
				});
			}
		...
		this.handleKeyboardForInputsInScroller = function() {
			if(this.keyboardHandled) return;
			return;
			var self = this;
...
*/
		...
		window.onpopstate = function(e) {
			var side = e.state.path; //we dont need to do anythign with it; pop state only works if something is stored though
			MobiRouter.back();
		};
