(function($) {
    MRouter = function(){
        ...
        /**
         * Open a sequence at a specified point.
         *
         * @param routesAndParams
         * @param pos
         * @param animate
         */
        /*this.openSequence = function(routesAndParams, pos, animate){
            var _this = this;
            if( _goingSomewhere == true || _animationRunning == true ){
                Meteor.setTimeout(function(){ _this.openSequence(routesAndParams, pos, animate); }, 50);
                return false;
            }

            _goingSomewhere = _animationRunning = true;
            pos = _.isNumber(pos) && pos < routesAndParams.length ? pos ? routesAndParams.length-1;

            var sequence = [],
                actPos = 0;
            _.each(routesAndParams, function(options, routeName){
                var r = _getRoute.call(_this, routeName);
                if( !r ) return false;

                var route = new MobiRoute(routeName, r);

                route.params = _.isObject(options) && _.isObject(options.params) ? options.params : {};
                route.position = actPos;
                route.preloaded = _.isObject(options) ? Boolean(options.preloaded) : false;
            ...
            _startNewSequence.call(this, route, sequence, true);
        ...
        function _startNewSequence(route, sequence, runAnyway){
            ...
            if( route && _.isFunction(route.before) )
                route.before();
        ...
        this.animateScroller = function(pos, time, afterAnimate, cb){
            ...
            if(_.isNumber(move) && _.isNumber(time) && document.getElementById('sequence_scroller') ){
                $('#sequence_scroller').hardwareAnimate({translateX: move}, time, this.settings.pageMoveEasing, function(el, animated){
                    ...
                    Meteor.setTimeout(function(){ _animationRunning = false; !animated ? parseInt(MobiRouter.settings.pageMoveTime) : 0});
                });
            }else
        ...
        this.alert = function(options){
            if( !_.isObject(options) ) return false;
            ...
            var _buttons = _.isArray(options.buttons) ? options.buttons : [],
                buttons = [];
