(function($) {
    MRouter = function(){
        ...
        this.content = function(route){
            ...
            if( !route ) return this.notFoundTemplate();

            if( route.type && route.type != 'SimplePage' ){
                var viewTemplate = _customViews[route.type];
                if( viewTemplate )
                    return Template[viewTemplate];
            }else if( route.type == undefined || route.type == 'SimplePage' ){
