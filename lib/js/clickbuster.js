(function(Meteor){
     ...
     cBuster = function(){
         ...
         this.onClick = function(event){
             ...
             if( event.type == 'click' && mobileScrolling && ['INPUT', 'TEXTAREA', 'SELECT', 'OPTION'].indexOf(event.target.tagName) == -1 ){
             ...
             for (var i = 0; i < preventedCoordinates.length; i += 2) {
                 ...
                 if (Math.abs(event.clientX - x) < 25 && Math.abs(event.clientY - y) < 25 && ['INPUT', 'TEXTAREA', 'SELECT', 'OPTION'].indexOf(event.target.tagName) == -1 ) {
                     event.preventDefault();
                     event.stopPropagation();
                     event.stopImmediatePropagation();
                     return false;
